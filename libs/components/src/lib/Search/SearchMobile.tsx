    ...
    <>
      <div className="relative flex items-center">
        ...
        {!isSearching && !isFocused && (
          <IconSearch onClick={focusInput} className="stroke-[#a1a1aa] absolute left-3.5 top-3" size={16} />
        )}
        {isFocused && (
          <IconArrowDown onClick={blurInput} className="stroke-[#a1a1aa] absolute left-3.5 rotate-90" size={18} />
        )}
        {!isSearching && isFocused && (
          <div onClick={clearInput} className="absolute right-1.5 h-8 w-8 flex justify-center items-center">
      ...
      {isFocused ? (
        <SearchList
          games={games}
          onNavigate={clear}
          className="border-b border-slate-800 w-screen right-0 top-[53px] bg-background/95 supports-backdrop-blur:bg-background/60 backdrop-blur h-[calc(100vh-118px)] overflow-y-scroll absolute"
        />
      ) : (
        <></>
      )}
