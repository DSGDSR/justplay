...
export const Avatar = () => {
      ...
      root.render(
        createPortal(
          <ViewProfile
            ...
            username={clerk.session?.user.username}
          />,
          userDropdown
        )
      );
  ...
  return (
    <div className="relative min-w-[2.25rem] h-9" id="user-avatar">
      ...
      <ClerkLoaded>
        ...
        <SignedOut>
          <Button size={'sm'} onClick={() => clerk.openSignIn({})} className="z-[1]">
            Sign in
          </Button>
        </SignedOut>
      </ClerkLoaded>
...
const ViewProfile = ({ classes, username }: any) => (
  <Link href={`/${username}`} className={classes.parent} role="menuitem">
    <div className={classes.icon}>
      <svg
        className={cn(classes.svg, 'w-4 h-4')}
        width="24"
        height="24"
        viewBox="0 0 24 24"
        strokeWidth="2.5"
