...
export const Carousel = ({ screenshots, auto = false, time = 3 }: Props) => {
  ...
  return (
    <div className="relative flex flex-col items-center aspect-video bg-background">
      <div className="w-full rounded overflow-x-hidden flex snap-x h-full object-contain" ref={slider}>
        {screenshots.map((s) => (
          <div key={s.image_id} className="snap-start w-full h-full flex-shrink-0" id={s.image_id}>
            <Image
      ...
      <div className="absolute w-full h-full flex justify-between items-center p-3.5">
        <button
          onClick={() => slide('prev')}
          className="p-0.5 bg-white text-black rounded-full cursor-pointer opacity-70 hover:opacity-100 transition-all"
        >
