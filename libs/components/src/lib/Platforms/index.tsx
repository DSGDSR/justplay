...
export const Platforms = ({ platforms, showTooltip = true }: PlatformsProps) => {
  ...
  return (
    <ul className="list-none flex items-center gap-3 sm:gap-4 text-slate-300 flex-wrap">
      {Object.keys(groupedPlatforms).map((platform) => (
        <li key={platform}>
          {showTooltip ? (
...
const PlatformTooltip = ({ platform }: { platform: IGroupedPlatforms }) => (
  <Tooltip>
    <TooltipTrigger asChild>
      <div className="scale-90 sm:scale-100">{PlatformIcons[platform.group]}</div>
    </TooltipTrigger>
