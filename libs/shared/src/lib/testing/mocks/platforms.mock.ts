export default [
  {
    id: 3,
    abbreviation: 'Linux',
    alternative_name: 'GNU/Linux',
    category: 4,
    created_at: 1297639288,
    name: 'Linux',
    platform_logo: 380,
    platform_family: 4,
    slug: 'linux',
    summary:
      'Linux is a free and open-source (FOSS/FLOSS) Operating System. In a more precise and complex definition, Linux is an open-source OS kernel on which a large variety of Operating Systems (known as Linux distributions) are built.',
    updated_at: 1392982362,
    url: 'https://www.igdb.com/platforms/linux',
    versions: [44],
    websites: [1],
    checksum: '4fdcdc95-88a3-8906-dbc8-d32e39a3df3a',
  },
  {
    id: 6,
    abbreviation: 'PC',
    alternative_name: 'mswin',
    category: 4,
    created_at: 1297639288,
    name: 'PC (Microsoft Windows)',
    platform_logo: 670,
    slug: 'win',
    updated_at: 1470063140,
    url: 'https://www.igdb.com/platforms/win',
    versions: [1, 13, 14, 15, 124, 513, 529, 530, 531, 532, 533, 534],
    websites: [2],
    checksum: 'e99646c7-dc88-795a-de4f-c7e07bc0839b',
  },
  {
    id: 14,
    abbreviation: 'Mac',
    alternative_name: 'Mac OS',
    category: 4,
    created_at: 1297678396,
    name: 'Mac',
    platform_logo: 761,
    slug: 'mac',
    updated_at: 1394314490,
    url: 'https://www.igdb.com/platforms/mac',
    versions: [45, 141, 142, 143, 144, 145, 146, 147, 148, 149, 150, 151, 598, 599, 600],
    websites: [5],
    checksum: '5d6e356f-9f06-0bfb-42d2-6eb89066135e',
  },
  {
    id: 34,
    abbreviation: 'Android',
    alternative_name: 'Infocusa3',
    category: 4,
    created_at: 1302608861,
    name: 'Android',
    platform_logo: 831,
    platform_family: 4,
    slug: 'android',
    updated_at: 1700445502,
    url: 'https://www.igdb.com/platforms/android',
    versions: [7, 8, 9, 10, 11, 12, 236, 237, 238, 239, 320, 526, 527, 528, 541, 542, 543, 544, 545, 546, 672],
    websites: [7],
    checksum: '82aba045-5c37-c12b-4a8f-0a74da66f37b',
  },
  {
    id: 39,
    abbreviation: 'iOS',
    category: 4,
    created_at: 1317719747,
    name: 'iOS',
    platform_logo: 248,
    slug: 'ios',
    updated_at: 1391704932,
    url: 'https://www.igdb.com/platforms/ios',
    versions: [43],
    checksum: '75b3e39f-688c-79bc-0023-a94553574fd6',
  },
  {
    id: 41,
    abbreviation: 'WiiU',
    alternative_name: 'Project Cafe',
    category: 1,
    created_at: 1317739102,
    generation: 8,
    name: 'Wii U',
    platform_logo: 239,
    platform_family: 5,
    slug: 'wiiu',
    updated_at: 1691513564,
    url: 'https://www.igdb.com/platforms/wiiu',
    versions: [73],
    websites: [10],
    checksum: '5026d37d-b0b2-4676-3fef-d3c2f3f085f1',
  },
  {
    id: 46,
    abbreviation: 'Vita',
    alternative_name: 'PS Vita',
    category: 5,
    created_at: 1321701445,
    generation: 8,
    name: 'PlayStation Vita',
    platform_logo: 232,
    platform_family: 1,
    slug: 'psvita',
    updated_at: 1552835771,
    url: 'https://www.igdb.com/platforms/psvita',
    versions: [60, 274, 275],
    websites: [34],
    checksum: '9a0e8a97-55f8-3f26-3549-36f6a68a7cfe',
  },
  {
    id: 48,
    abbreviation: 'PS4',
    alternative_name: 'PS4',
    category: 1,
    created_at: 1326544944,
    generation: 8,
    name: 'PlayStation 4',
    platform_logo: 231,
    platform_family: 1,
    slug: 'ps4--1',
    summary:
      'The PlayStation 4 system opens the door to an incredible journey through immersive new gaming worlds and a deeply connected gaming community. PS4 puts gamers first with an astounding launch line-up and over 180 games in development. Play amazing top-tier blockbusters and innovative indie hits on PS4. Developer inspired, gamer focused.',
    updated_at: 1433175597,
    url: 'https://www.igdb.com/platforms/ps4--1',
    versions: [17, 178, 179],
    websites: [11],
    checksum: '3e517e16-442d-e91b-e416-4fcb2733c9b2',
  },
  {
    id: 49,
    abbreviation: 'XONE',
    alternative_name: 'XONE',
    category: 1,
    created_at: 1326545022,
    generation: 8,
    name: 'Xbox One',
    platform_logo: 329,
    platform_family: 2,
    slug: 'xboxone',
    summary:
      'Welcome to a new generation of games and entertainment. Where games push the boundaries of realism. And television obeys your every command. Where listening to music while playing a game is a snap. And you can jump from TV to movies to music to a game in an instant. Where your experience is custom tailored to you. And the entertainment you love is all in one place. Welcome to the all-in-one, Xbox One',
    updated_at: 1633117143,
    url: 'https://www.igdb.com/platforms/xboxone',
    versions: [78, 180, 185, 188],
    websites: [12],
    checksum: 'fa077139-6df3-2516-cf1c-a8f0ba7fb4a7',
  },
  {
    id: 130,
    abbreviation: 'Switch',
    alternative_name: 'NX',
    category: 1,
    created_at: 1465948800,
    generation: 8,
    name: 'Nintendo Switch',
    platform_logo: 534,
    platform_family: 5,
    slug: 'switch',
    updated_at: 1550534400,
    url: 'https://www.igdb.com/platforms/switch',
    versions: [173, 282],
    websites: [16],
    checksum: 'd28b6757-c24a-73c6-50e7-f8e6487c78af',
  },
];
