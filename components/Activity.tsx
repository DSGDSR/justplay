    ...
    return isClient ? <ul className="-my-2">
        {
            (limit ? activity.slice(0, limit) : activity).map((item, idx) => {
                ...
                return <li key={`${item.game}-${item.list_type}`} className="relative pl-8 sm:pl-[7.5rem] py-3.5 group">
                    <div className="flex flex-col sm:flex-row items-start group-last:before:hidden before:absolute group-hover:after:bg-slate-800 before:left-2 sm:before:left-0 before:h-full before:px-px before:bg-slate-700 sm:before:ml-24 before:self-start before:-translate-x-1/2 before:translate-y-3 after:absolute after:left-2 sm:after:left-0 after:w-3.5 after:h-3.5 after:bg-slate-700 after:rounded-full sm:after:ml-24 after:-translate-x-1/2 after:translate-y-1.5">
                        <Badge variant="secondary" className="bg-slate-700 group-hover:bg-slate-800 sm:absolute left-0 mb-3 sm:mb-0 sm:w-[4.5rem] justify-center text-center text-[.7rem]">
                            { from('en', new Date(item.created_at)) }
                        </Badge>
                        <div>
                            {ActivityItems[item.list_type]({ game, list })}
                            {games.find(g => g.object.id === item.game)?.object.name}, {customLists.find(cl => cl.id === item.custom_list_id)?.name}
                        </div>
                    </div>
...
const ActivityItems: Record<ActivityTypes, (props: ActivityItemProps) => JSX.Element> = {
    [ActivityTypes.Favorite]: ({ game, list }: ActivityItemProps) => <>{game.name}</>,
    [ActivityTypes.Playlist]: ({ game, list }: ActivityItemProps) => <>{game.name}</>,
    [ActivityTypes.Finished]: ({ game, list }: ActivityItemProps) => <>{game.name}</>,
    [ActivityTypes.Custom]: ({ game, list }: ActivityItemProps) => <>
        {list?.name}, {game.name}
    </>
}
